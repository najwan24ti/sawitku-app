...
<head>
    ...
    <style>
        ...
        .navbar-brand { color: var(--accent-green) !important; }

        /* Theme Toggle Button */
        .theme-toggle-btn {
            border: none; background: none; cursor: pointer; font-size: 1.3rem; color: var(--text-main);
...
<body>
    ...
    <nav class="navbar navbar-expand-lg sticky-top py-3 shadow-sm z-3">
        <div class="container">
            <a class="navbar-brand fw-bold d-flex align-items-center gap-2" 
               href="{{ Auth::check() ? route('guest.dashboard') : route('home') }}">
                <i class="fas fa-seedling fa-lg"></i> SawitKu
            </a>
    ...
    <nav class="navbar fixed-bottom navbar-light bg-white border-top d-block d-lg-none p-0 shadow-lg z-3" style="padding-bottom: env(safe-area-inset-bottom); background-color: var(--bg-card) !important; border-color: var(--border-color) !important;">
        <div class="container-fluid d-flex justify-content-around py-2">
            <a href="{{ route('guest.dashboard') }}" class="text-decoration-none text-center {{ request()->routeIs('guest.dashboard') ? 'text-success' : 'text-secondary' }}">
    ...
    <script>
        ...
        themeToggle.addEventListener('click', () => {
            ...
            applyTheme(newTheme);
            if(typeof Chart !== 'undefined') location.reload(); // Reload chart if exists
        });
